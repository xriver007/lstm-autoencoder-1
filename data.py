...
def shuffle_and_index(data_dir):
    ...
    for i, f in enumerate(files):
        ...
        new = os.path.join(data_dir, '{}{}{}'.format(
            i, DELIMITER, f.split(DELIMITER)[1] if DELIMITER in f else f
        ))
        os.rename(old, new)
...
def filename_by_index(data_dir, index):
    ...
    files = os.listdir(data_dir)
    assert all([f.split(DELIMITER)[0].isdigit() for f in files]), 'data not indexed'
    files = sorted(files, key=lambda f: int(f.split(DELIMITER)[0]))
